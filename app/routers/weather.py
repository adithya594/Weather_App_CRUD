...
def view(request: Request, location: str, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(            "view.html",            {                "request": request,                "place": place,                "lat": lat,                "lon": lon,                "current": current,                "now_icon_url": f"https://openweathermap.org/img/wn/{now_icon}@2x.png",                "days": [                    {                        "date": f.date.isoformat(),                        "min": f.min_temp,                        "max": f.max_temp,                        "main": "",                        "icon": "",                    }                    for f in forecasts                ],            },        )
